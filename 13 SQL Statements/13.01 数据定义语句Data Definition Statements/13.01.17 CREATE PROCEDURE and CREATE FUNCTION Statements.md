...
这些语句用于创建存储例程(存储过程或函数)。也就是说，服务器知道指定的例程。
...
请注意：**将参数指定为 IN、OUT 或 INOUT 仅对过程有效**。对于一个函数来说，参数总是被视为 IN 参数。
...
**RETURNS 子句只能为一个函数指定，对于这个函数它是必须的**。
...
如果在例程定义中既没有给出确定性也没有给出确定性，则**默认值是不确定性的**。
